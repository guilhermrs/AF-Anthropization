...
MintElvSlpEco<- gls(X_antropiza~Elevation_+Slope_mean+ECO_NAME:Elevation_+ECO_NAME:Slope_mean, data = dados_ecoregioes, correlation = corExp(form = ~Latitude + Longitude), method = "REML")  # = "modelo_corExp_ecoregion"
...
# Plot using your preferred design
Fig2_forestplot_ecoregion <- ggplot(df_long, aes(x = Coefficient, y = Ecoregion, color = Variable)) +
  geom_point(size = 3) +
...
Fig3_Slope_Ecoregion <- ggplot(dados_ecoregioes_controlando_elevation, aes(x = Slope_mean, y = predictions, color = ECO_NAME)) +
  ...
  facet_wrap(~ ECO_NAME, ncol = 2, scales = "free_y") +  # Independent y-axes for each ecoregion
  scale_color_viridis_d(option = "D") + 
  theme_minimal() +
  theme(
...
Fig4_Elevation_Ecoregion <- ggplot(dados_ecoregioes_controlando_slp, aes(x = Elevation_, y = predictions, color = ECO_NAME)) +
  geom_point(aes(y = partial), shape = 16, size = 0.7, alpha = 0.8) +  # Points: residuals + predictions
  geom_smooth(method = "lm", se = FALSE, color = "black", size = 1) +  # Lines: predicted values
  labs(
    x = "Elevation (m)", 
